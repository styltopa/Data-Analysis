...
title({['Diagnostic plot: ',...
    'Normalised errors'];'Full parameter linear regression model'});
fontSize = 15;
...
title({['Diagnostic plot: ',...
    'Normalised errors'];'Stepwise linear regression model'});
fontSize = 15;
xlabel('$y_i$', 'interpreter', 'latex', 'FontSize', fontSize);
ylabel('Normalised errors $e_i^*$', 'interpreter', 'latex', 'FontSize', ...
    fontSize);
...
%% Comparison, remarks
fprintf(['Full parameter linear regression model:\nR2: ',...
    '%f\nadjR2: %f\n\n'], R2, adjR2);
